...
static void led_strip_task(void *arg)
{
    ...
    led_strip->rmt_items = (rmt_item32_t*) malloc(sizeof(rmt_item32_t) * num_items_malloc);
    if (!led_strip->rmt_items) {
        vTaskDelete(NULL);
    }
    ...
    for(;;) {
        ...
        if (make_new_rmt_items) {
            if (led_strip->showing_buf_1) {
                led_make_waveform(led_strip->led_strip_buf_1, led_strip->rmt_items, led_strip->led_strip_length);
            } else {
                led_make_waveform(led_strip->led_strip_buf_2, led_strip->rmt_items, led_strip->led_strip_length);
            }
        }

        rmt_write_items(led_strip->rmt_channel, led_strip->rmt_items, num_items_malloc, false);
...
bool led_strip_init(struct led_strip_t *led_strip)
{
    ...
    if(led_strip->led_strip_buf_1 == led_strip->led_strip_buf_2) {
        printf("Led strip buffers joined\n");
        return false;
    }
    ...
    led_strip->task_handle = led_strip_task_handle;
