...
(
   ...
   m_initialized(false)
{
   ESP_LOGI(TAG,"Created on port %i and address %#x", _port, _address);
   initialize();
}
...
bool BME280::ReadChipID()
{
   ...
   ESP_LOGI(TAG,"Reading Chip ID");

   esp_err_t res = readRegister(ID_ADDR, &id[0], 1);
   ...
   if (res == ESP_OK) {
      ...
      switch(id[0])
      {
         ...
         case ChipModel_BMP280:
            ...
            break;
         default:
            ESP_LOGI(TAG, "Found Unkown %#x", id[0]);
...
bool BME280::ReadTrim()
{
   ...
   // Temp. Dig
   success &= (readRegister(TEMP_DIG_ADDR, &m_dig[ord], TEMP_DIG_LENGTH) == ESP_OK);
   ord += TEMP_DIG_LENGTH;

   // Pressure Dig
   success &= (readRegister(PRESS_DIG_ADDR, &m_dig[ord], PRESS_DIG_LENGTH) == ESP_OK);
   ord += PRESS_DIG_LENGTH;

   // Humidity Dig 1
   success &= (readRegister(HUM_DIG_ADDR1, &m_dig[ord], HUM_DIG_ADDR1_LENGTH) == ESP_OK);
   ord += HUM_DIG_ADDR1_LENGTH;

   // Humidity Dig 2
   success &= (readRegister(HUM_DIG_ADDR2, &m_dig[ord], HUM_DIG_ADDR2_LENGTH) == ESP_OK);
...
)
{
   ...
   success = (readRegister(PRESS_ADDR, buffer, SENSOR_DATA_LENGTH) == ESP_OK);
