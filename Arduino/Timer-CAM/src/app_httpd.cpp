...
#if CONFIG_ESP_FACE_DETECT_ENABLED
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
static void rgb_print(dl_matrix3du_t *image_matrix, uint32_t color,
                      const char *str) {
    fb_data_t fb;
...
static int rgb_printf(dl_matrix3du_t *image_matrix, uint32_t color,
                      const char *format, ...) {
    char loc_buf[64];
    char *temp = loc_buf;
    int len;
...
#if CONFIG_ESP_FACE_RECOGNITION_ENABLED
static int run_face_recognition(dl_matrix3du_t *image_matrix,
    ...
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK) {
        if (is_enrolling == 1) {
            ...
            ESP_LOGD(TAG, "Enrolling Face ID: %d sample %d", id_list.tail,
...
static esp_err_t win_handler(httpd_req_t *req) {
    ...
    ESP_LOGI(TAG,
             "Set Window: Start: %d %d, End: %d %d, Offset: %d %d, Total: %d "
             "%d, Output: %d %d, Scale: %u, Binning: %u",
